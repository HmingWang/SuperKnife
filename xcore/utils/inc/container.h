...
// 序列容器概念（支持push_back）
template <typename C>
concept SequenceContainer = ContainerType<C> && requires(C c, typename C::value_type v) {
...
template <typename C>
concept FrontInsertionContainer = SequenceContainer<C> && requires(C c, typename C::value_type v) {
...
// 关联容器概念
template <typename C>
concept AssociativeContainer = ContainerType<C> && requires(C c, typename C::key_type k) {
    typename C::key_type;
...
public:
    ...
    // 6. 移动构造函数
    Container(Container &&) noexcept(std::is_nothrow_move_constructible_v<T>) = default;
    // 7. 拷贝赋值运算符
...
// 特化 std::swap
template <ContainerType T>
void swap(Container<T> &lhs, Container<T> &rhs) noexcept(noexcept(lhs.swap(rhs)))
{
    lhs.swap(rhs);
}
