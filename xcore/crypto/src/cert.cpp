...
namespace x::crypto
{
  ...
  Cert Cert::Generator::create_self_signed(PKey &key, std::string_view subject, const EVP_MD *md, int vaild_days)
  {
    ...
    Utils::add_extensions(cert.get(), NID_key_usage, "critical,keyCertSign,cRLSign");
  ...
  Cert Cert::Generator::create_from_csr(CSR &req, PKey &cakey, Cert &caCert, const EVP_MD *md, int vaild_days)
  {
    X509_ptr cert(X509_new());
    OSSL_ASSERT_PTR(cert);
    OSSL_ASSERT_FUNC(X509_set_version(cert.get(), 2));
    ...
    // 设置主题名称 (从请求中复制)
    OSSL_ASSERT_FUNC(X509_set_subject_name(cert.get(), X509_REQ_get_subject_name(req.get_X509_REQ())));

    // 设置颁发者名称 (CA的名称)
    OSSL_ASSERT_FUNC(X509_set_issuer_name(cert.get(), X509_get_subject_name(caCert.get_X509())));
    OSSL_ASSERT_FUNC(X509_set_pubkey(cert.get(), X509_REQ_get_pubkey(req.get_X509_REQ())));

    // 添加扩展
    Utils::add_extensions(cert.get(), NID_basic_constraints, "critical,CA:FALSE");
    Utils::add_extensions(cert.get(), NID_key_usage, "critical,digitalSignature,keyEncipherment");
    Utils::add_extensions(cert.get(), NID_ext_key_usage, "serverAuth,clientAuth");
    Utils::add_extensions(cert.get(), NID_subject_key_identifier, "hash");
    Utils::add_extensions(cert.get(), NID_authority_key_identifier, "keyid:always");
  ...
  void Cert::save(std::string_view filename)
  {
  ...
  void Cert::print_info()
  {
    print_basic();
    print_extensions();
    print_finger_points();
  }
  void Cert::print_basic()
  {
