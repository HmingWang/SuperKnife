...
bool Cert::createSelfSigned(KeyPair &keyPair, const EVP_MD *md,
                            const string &subject, int validDays) {
...
Cert Cert::signedCertificate(CertReq &req, KeyPair &caKeyPair, const EVP_MD *md,
                             int validDays) {
  ...
  if (X509_gmtime_adj(X509_get_notAfter(cert_.get()), validDays * 24 * 3600) ==
      nullptr) {
  ...
  if (X509_set_subject_name(
          cert_.get(),
          X509_REQ_get_subject_name(req.getCertificateRequest())) != 1) {
...
bool CertReq::createCertificateRequest(KeyPair &keyPair,
