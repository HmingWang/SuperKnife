...
bool SM2KeyPair::generateKeyPair() {
  unique_ptr<EVP_PKEY_CTX, decltype(&EVP_PKEY_CTX_free)> ctx(
      EVP_PKEY_CTX_new_id(EVP_PKEY_SM2, nullptr), &EVP_PKEY_CTX_free);
  if (!ctx)
    return false;

  EVP_PKEY_CTX_set1_id(ctx.get(), "1234567812345678", 16);
...
SM2Crypto::encrypt(EVP_PKEY *publicKey,
...
vector<unsigned char>
SM2Crypto::decrypt(EVP_PKEY *privateKey,
